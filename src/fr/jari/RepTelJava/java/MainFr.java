...
public class MainFr {
    ...
    public static void displayWrite(mainGUI f, WriteFr w) throws InterruptedException {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        while(true) {
            ...
            f.label2.setText("Fichier actuel:  Unknown");
            ...
            if (listOfFiles == null){
                files.append("<br/ ").append(" Pas de fichiers");
            } else {
            ...
            if (!hasFiles) { // New file since no pre-existing file
                f.setConsoleOutput("> Pas de fichiers existants", RED);
                f.label1.setText("Entrez un nom de fichier à créer: ");
                while (!f.isPressed()) {
                    Thread.sleep(100);
                } if (f.isPressed()) {
            ...
            if (hasFiles){ // Selection of the file
                while (true) {
                    f.label1.setText("<html> A quel fichier voulez-vous accéder? " + files + " </html>");
                    while (!f.isPressed()) {
                        Thread.sleep(100);
                    }
                    if (f.isPressed()) {
                        ...
                        if (filesList.contains(file)) {
                            ...
                            f.setConsoleOutput(("> " + file + ".txt n'existe pas, le créer?"), RED);
                            String newFile = f.setChoiceBox(new ArrayList<>( Arrays.asList("Oui", "Non")), false);
                            if (newFile.equals("Oui")){
                                f.label1.setText("Entrez le nom du fichier à créer: ");
                                while (!f.isPressed()) {
                                    Thread.sleep(100);
                                } if (f.isPressed()) {
                                    String fileName = f.input;
            ...
            while (true) {
                // Menu display
                f.label2.setText("Fichier actuel:  " + w.getFilename());
                f.label1.setText("<html>0 - Quitter <br/> 1 - Ecrire <br/> 2 - Rechercher <br/> 3 - Récupérer le nombre de personnes " +
                        "<br/> 4 - Créer un nouveau fichier <br/> 5 - Changer de fichier </html>");
                ...
                switch (choice) {
                    ...
                    case "1" -> { // Write in the file
                        ...
                        while (true) {
                            ...
                            f.label1.setText("Entrez un nom (0 pour quitter): ");
                            while (!f.isPressed()) {
                                Thread.sleep(100);
                            }
                            if (f.isPressed()) {
                                ...
                                f.label1.setText("Entrez un nom: " + name);
                                ArrayList<String> filecontent = r.read(myReader);
                                if (filecontent.contains(name)){
                                    String add = f.setChoiceBox(new ArrayList<>(Arrays.asList("Oui", "Non")), "> Nom déjà assigné, rajouter un numéro? Oui/Non", RED);
                                    f.setPressed(false);
                                    if (add.equals("Oui")){
                                        ...
                                        for (String i : filecontent.subList(start, filecontent.size())) {
                                            if (found) {
                                                if (!testType(i)) {
                                                    ...
                                                    f.consoleOutput.setText("> ");
                                                    f.label1.setText("<html> " + f.label1.getText() + " <br/> Entrez un numéro: </html>");
                                                    while(!f.isPressed()){
                                                        Thread.sleep(100);
                                                    } if (f.isPressed()){
